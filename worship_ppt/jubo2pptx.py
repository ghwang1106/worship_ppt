...
class Jubo():
  ...
  def grab_section(self, s_start, s_finish):
    if type(s_finish) == int:
      return self.all_text[self.find_index(self.all_text, s_start)[0]:self.find_index(self.all_text, s_start)[0] +
                           s_finish]
    else:
...
class PPT:
  ...
  def make_worship2(self, title):
    ...
    title = title.split('\n')
    if len(title) != 2:
      logger.warning('Unexpected number of lines in the title page')
