...
class Hymn():
    ...
    def lookup(self, song_type='hymn', number=0, title=''):
        try:
            song_received = next(item for item in self.all if item['song_type'] == song_type and item['number'] == number and title in item['title'])
...
class Jubo():
        ...
        for block in self.iter_block_items(jubo_raw):
            if isinstance(block, Table):
                for row in block.rows:
                    ...
                    if row_data:
                        row_data = self.remove_dup(row_data)
    ...
    def remove_dup(self, row_data):
        no_dup = []
        [no_dup.append(i) for i in row_data if not no_dup.count(i)]
        return no_dup

    def grab_section(self, s_start, s_finish):
        if type(s_finish) == int:
            return self.all_text[self.find_index(self.all_text, s_start)[0]:self.find_index(self.all_text, s_start)[0]+s_finish]
        else:
            return self.all_text[self.find_index(self.all_text, s_start)[0]:self.find_index(self.all_text, s_finish)[0]]
...
class PPT:
    ...
    def __init__(self, layout=[10, 7.5]):
        self.layout = layout
        self.prs = Presentation()
        self.prs.slide_width, self.prs.slide_height = Inches(layout[0]), Inches(layout[1])

    def make_announcement(self, announcement):
        for s in announcement:
            if len(s) == 0:
                pass
            elif s[0] in ['-', '*'] or re.search('^\S*\:', s):
                self.add_paragraph(s.replace("  ", "\n\n"), 28, spacing=1.1)
            else:
                if "  " in s:
                    print(s)
    ...
    def make_worship1(self, worship_order, hymn):
        ...
        for s in worship_order:
            ...
            if s in to_ignore:
                ...
                d = [3.09, 3.7, 6.8, 3.76]
                creed_text = ['신앙고백-사도신경', '전능하사 천지를 만드신\n하나님 아버지를 내가 믿사오며,\n그 외아들 우리 주 예수\n그리스도를 믿사오니,',
                              '이는 성령으로 잉태하사\n동정녀 마리아에게 나시고,\n본디오 빌라도에게 고난을 받으사,\n십자가에 못 박혀 죽으시고',
                              '장사한지 사흘 만에\n죽은 자 가운데서 다시 살아나시며,\n하늘에 오르사 전능하신 하나님\n우편에 앉아 계시다가,',
                              '저리로서 산 자와 죽은 자를\n심판하러 오시리라.\n성령을 믿사오며 거룩한 공회와,\n성도가 서로 교통하는 것과,',
                              '죄를 사하여 주시는 것과,\n몸이 다시 사는 것과,\n영원히 사는 것을 믿사옵나이다.\n아멘']
                self.add_textbox(creed_text[0], [sum(x) for x in zip(d, [0, -0.84, 0, 0])], 32)
                self.add_paragraph('', 5)
    ...
    def make_worship2(self, title):
        ...
        if len(title) is not 2:
            print('  >> WARNING: Unexpected number of lines in the title page')
    ...
    def add_textbox(self, text, dim, fontsize, fontname='Malgun Gothic', spacing=1.1, bold=True, align=PP_ALIGN.CENTER, newslide=1):
        if newslide:
            self.add_slide()
    ...
    def text_length(self, v, middle=0):
