...
class Bible:
    ...
    def __init__(self, json_file='bible.json'):
    ...
    def lookup(self, verses_tf):
        ...
        for i in range(len(verses_itr)):
            ...
            verses_itr[i] = [bb.Abbr[int(bb[bb.Eng == verses_itr[i][0]].index[0])]] + verses_itr[i]
...
class Sermon:
    ...
    def __init__(self, input_file='bible_ppt_prep.txt'):
        with open(input_file, 'r', encoding='utf-8') as f:      # read prep text file
            raw_inputs = f.read().split('\n')

        if len(raw_inputs) > 4:
            if raw_inputs[4].isspace() or raw_inputs[4] == '':  # Remove extra lines
                raw_inputs = raw_inputs[:4]
                print('  >> WARNING: Remove spaces in the 5th line in the prep file')
            else:
                if len(raw_inputs) > 5:
                    print('  >> WARNING: Remove extra lines after 5th line in the prep file')
                raw_inputs = raw_inputs[:5]
    ...
    def passages_raw2ind(self, passages_raw):
        ...
        for i in range(len(passages_raw)):
            ...
            if book in list(bb.Abbr):      # if only abbreviation of the book is given, turn it into full name
                passages_raw[i] = bb.Full[int(bb[bb.Abbr == book].index[0])] + ' ' + verses
    ...
    def parse_passage(self, passage):
        try:
            ...
            book_ind = bb.Eng[int(bb[bb.Full == book].index[0])]
...
class PPT:
    ...
    def __init__(self, layout=[12192000, 6858000]):     # 13.33, 7.5 inches
        ...
        if os.path.exists('template.pptx'):
            ...
            print('  >> WARNING: template.pptx missing (pptx will not have \"browsed by individual window\" setting turned on')
    ...
    def create_large(self, sermon):     # Large.pptx format
        self.add_large_slides(sermon.passages_ext, 7)
    ...
    def add_large_slides(self, passage, n_lines):
    ...
    def to_pptx(self, path, dup=False):
        try:
            self.prs.save(path + '.pptx')   # save to pptx with specified path
            os.startfile(path + '.pptx')              # open saved file to check
        except PermissionError:
            if dup:
                self.to_pptx(path + '1', dup=True)
            else:
                sys.exit('  >> ERROR: Cannot save to file because it is open')
...
if not parser.parse_args().v:
    ppt_large = PPT()
    ppt_large.create_large(sermon)
    ppt_large.to_pptx(re.sub("\D+", "_", sermon.date_type)+'Large')

ppt_verse = PPT()
ppt_verse.create_verse(sermon)
if parser.parse_args().v:
    ppt_verse.to_pptx(re.sub("\D+", "_", sermon.date_type)+'Bible', dup=True)
else:
    ppt_verse.to_pptx(re.sub("\D+", "_", sermon.date_type)+'Bible')
