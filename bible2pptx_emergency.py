...
class Bible:
  def __init__(self, bb, json_file='./worship_ppt/data/bible.json'):
    self.bb = bb
...
class PPT:
  def __init__(self, layout=[12192000, 6858000]):     # 13.33, 7.5 inches
    self.layout = layout
    if os.path.exists('./worship_ppt/data/template.pptx'):
      self.prs = Presentation('./worship_ppt/data/template.pptx')    # template file with "browsed by individual window" turned on
  ...
  def add_large_slides(self, sermon, passage, n_lines):
    ...
    for v in sermon.passages_ext:
      ...
      if lc > n_lines:    # create new page for every 7 lines
        self.add_slide()
        self.add_background('./worship_ppt/data/bible_background.jpg', sermon.date_type, sermon.passages_raw[0], './worship_ppt/data/church_logo.png')
...
bb = pd.read_csv('./worship_ppt/data/bible_info.csv')
bible = Bible(bb)
